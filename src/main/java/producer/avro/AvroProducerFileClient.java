...
private static final String bootstrapServers = "192.168.56.2:9092";
private static final String schemaRegistryURL = "http://192.168.56.2:8081";
private static final String topic = "nsefo-topic-avro";
private static final String filePath = "D:/tmp/fo02JAN2020bhav.csv";
    ...
    private String convertStringToISODateString(String str) {
        String mname = str.substring(3, 6).toUpperCase();
    ...
    private void runProducer() throws IOException {
        KafkaProducer<String, Nseforec> producer = new KafkaProducer<String, Nseforec>(createProducerConfig());
...
//        new AvroProducerFileClient().runProducer(args[0], args[1], args[2], args[3]);
        new AvroProducerFileClient().runProducer();
    }

    private String createKey(String rval) {
        String[] splits = rval.split(",");
        String key = splits[1] +
                convertStringToISODateString(splits[2]) +
                convertStringToISODateString(splits[14])
                + splits[0] + splits[4] + splits[3];
//        System.out.println("Record value: " + rval + " and key: " +key);
        return key;
    }
